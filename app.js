(()=>{
    const App = {
        ...
        definicionDeEventos(){
            App.elementosHtml.botones.forEach(boton => {boton.addEventListener('click',App.metodos.validarDato)});
        },
        metodos: {
            pantallaActual: '',
            pantallaAnterior: '',
            ...
            validarDato(event){
                ...
                if (!isNaN(valorBotonDataSet) || valorBotonDataSet === '.' || valorBotonDataSet === '-' || valorBotonDataSet === '+'|| valorBotonDataSet === 'x' || valorBotonDataSet === '/' || valorBotonDataSet === '=' || valorBotonDataSet === 'c') {
                    switch (valorBotonDataSet) {
                        case '+':
                        case '-':
                        case 'x':
                        case '/':
                        ...
                        default:
                            if(!isNaN(App.metodos.pantallaActual) && !isNaN(App.metodos.pantallaAnterior)){
                                if (App.metodos.pantallaAnterior === '' && App.metodos.pantallaActual === '') {
                                    App.metodos.pantallaActual =  valorBotonDataSet;                          
                                }
                                else{
                                    App.metodos.pantallaActual += valorBotonDataSet;
                                }
            ...
            realizarCalculo(){
                switch (App.metodos.operador) {
                    ...
                    case 'x':
                        ...
                        break;
                    case '/':
                        if (parseFloat(App.metodos.pantallaActual) === 0) {
                            App.metodos.resultado = 'No válido';
                        }
                        if (App.metodos.pantallaActual === '') {
                            App.metodos.pantallaActual = '';
                        }
